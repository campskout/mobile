...
const Profile = () => {
  ...
  const handleAccept = async (userId: string, postId: string) => {
    try {
      await axios.post(`http://192.168.10.6:5000/api/acceptAndReject/${userId}/${postId}`);
  ...
  const handleReject = async (userId: string, postId: string) => {
    try {
      await axios.post(`http://192.168.10.6:5000/api/acceptAndReject/reject/${userId}/${postId}`);
      setParticipants(prevParticipants =>
        prevParticipants.map(participant =>
          participant.userId === userId && participant.postId === postId
